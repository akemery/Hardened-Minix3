			...
			int is_nested)
{
	...
	int err;
        ...
        /** The page fault was already handled
         ** by check_vaddr_2
         ** The native Minix 3 handler
         ** will just return **/
        if(h_rw == K_HANDLE_PF)
           return;

        /** VM should handle the page fault. That 
         ** is a page fault caused by the
         ** hardening. Inform the VM. The VM will 
         ** allocate a frame for US1 or 
         ** US2 and will create data structure to 
         ** handle that page in its own 
         ** address space.
         ** The variable h_step will be changed 
         ** to allow the VM to run when 
         ** hardening is enable. Its previous value
         ** is saved in h_step_back. 
         ** Its new value is VM_RUN. The message 
         ** to the VM will be changed too.
         **/
        if((h_rw == RO_PAGE_FIRST_RUN) ||
            (h_rw == RO_PAGE_SECOND_RUN)){
  ...
  /** End by EKA to support hardening**/
	in_physcopy =
              (frame->eip > (vir_bytes) phys_copy) &&
	   (frame->eip < (vir_bytes) phys_copy_fault);

	in_memset =
           (frame->eip > (vir_bytes) phys_memset) &&
	...
	if((is_nested || iskernelp(pr)) &&
		...
		if(in_physcopy) {
		  assert(!in_memset);
		 frame->eip = 
                   (reg_t) phys_copy_fault_in_kernel;
		} else {
		 frame->eip = 
                   (reg_t) memset_fault_in_kernel;
		}
	   }
	   else {
	...
	if(is_nested) {
	   printf("pagefault in kernel at pc "
             "0x%lx address 0x%lx\n",
			frame->eip, pagefaultcr2);
	   inkernel_disaster(pr, frame, 
             NULL, is_nested);
	}
	...
	if(pr->p_endpoint == VM_PROC_NR) {
	  ...
	  */
	  printf("pagefault for VM on CPU %d, "
	    "pc = 0x%x, addr = 0x%x, flags = 0x%x, "
            "is_nested %d\n",
	    cpuid, pr->p_reg.pc, 
            pagefaultcr2, frame->errcode,
	    is_nested);
	  proc_stacktrace(pr);
	...
	/* Don't schedule this process until
         * pagefault is handled. */
        ...
        if(changemsg2vm == H_NO)
           ...
           switch(h_rw) {
               case RO_PAGE_FIRST_RUN :   
                  m_pagefault.m_type   = 
                    VM_HR1PAGEFAULT; break;
               case RO_PAGE_SECOND_RUN:   
                  m_pagefault.m_type   = 
                      VM_HR2PAGEFAULT; break;
               default : 
                 panic("case not supported %d", h_rw);
           }
        }
        /***End Add by EKA***/
        if ((err = mini_send(pr, VM_PROC_NR,
		&m_pagefault, FROM_KERNEL))) {
		panic("WARNING: pagefault: "
  ...
  if (is_nested) {
	...
	*/
	...
	return;
       ...
       /* If control passes, this case is not 
