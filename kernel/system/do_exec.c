 ...
 *   m_lsys_krn_sys_exec.endpt		
 *     (process that did exec call)
 *   m_lsys_krn_sys_exec.stack		
 *     (new stack pointer)
 *   m_lsys_krn_sys_exec.name		
 *     (pointer to program name)
 *   m_lsys_krn_sys_exec.ip		
 *     (new instruction pointer)
 *   m_lsys_krn_sys_exec.ps_str		
 *      (struct ps_strings *)
 */
...
{
/* Handle sys_exec().  A process has done
 * a successful EXEC. Patch it up. */
  ...
  if(!isokendpt(
     m_ptr->m_lsys_krn_sys_exec.endpt, &proc_nr))
	return EINVAL;
