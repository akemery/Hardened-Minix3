 ...
 *		pt_writemap	                *
 *==============================================*/
/* path: scr/minix/servers/vm/pagetable.c*/
 ...
 * for this virtual address. */
   ...
   */
   ...
   /* End added by EKA*/
   /* MAP_NONE means to clear the mapping. 
    * It doesn't matter
    ...
    * MAP_NONE into it.
    */
    assert(physaddr == MAP_NONE || 
          (flags & ARCH_VM_PTE_PRESENT));
    ...
    */
    ret = pt_ptalloc_in_range(pt, v, 
          v + VM_PAGE_SIZE*pages, flags, verify);
    if(ret != OK) {
       printf("VM: writemap:"
...
#elif defined(__arm__)
	physaddr = 
             pt->pt_pt[pde][pte] & ARM_VM_PTE_MASK;
#endif
      }
...
#ifdef CONFIG_SMP
   if (vminhibit_clear) {
     assert(vmp && vmp->vm_endpoint != NONE && 
            vmp->vm_endpoint != VM_PROC_NR &&
	    !(vmp->vm_flags & VMF_EXITING));
     sys_vmctl(vmp->vm_endpoint, 
