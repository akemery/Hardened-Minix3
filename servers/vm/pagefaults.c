...
static void handle_memory_continue(struct vmproc *vmp, 
             message *m, void *arg, void *statearg);

static void handle_pagefault(endpoint_t ep, vir_bytes addr, 
                              u32_t err, int retry, int m_type)
{
	struct vmproc *vmp;
	int s, result;
	...
	if(!(region = map_lookup(vmp, addr, NULL))) {
              /** Since GLOBAL page is not in the working
                  that event never happen**/
		if(PFERR_PROT(err))  {
        ...
        void *arg, void *statearg)
{
	...
	handle_pagefault(state->ep, state->vaddr, state->err, 1, 0);
}
...
{
	handle_pagefault(m->m_source, m->VPF_ADDR, m->VPF_FLAGS, 0, m->m_type);
}
