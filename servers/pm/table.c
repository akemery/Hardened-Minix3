...
int (* const call_vec[NR_PM_CALLS])(void) = {
	...
	CALL(PM_GETSYSINFO)	= do_getsysinfo,		/* getsysinfo(2) */
        CALL(PM_ENABLE_HARDENING)  = do_enable_hardening, /* enable_hardening() */
        CALL(PM_DISABLE_HARDENING) = do_disable_hardening, /* enable_hardening() */
        CALL(PM_HARDENING)         = do_hardening /* hardening() */
};
